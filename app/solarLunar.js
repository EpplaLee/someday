...
let sum = 348
  ...
  getTerm(y, n) {
    ...
    const _table = solarLunar.lTermInfo[y - 1900]
    const _info = [
  ...
  toChinaYear(y) {
    // 年 => \u5E74
    const oxxx = parseInt(y / 1000)
    const xoxx = parseInt((y % 1000) / 100)
    const xxox = parseInt((y % 100) / 10)
    const xxxo = y % 10

    return (
      `${solarLunar.nStr4[oxxx] +
      solarLunar.nStr4[xoxx] +
      solarLunar.nStr4[xxox] +
      solarLunar.nStr4[xxxo] 
      }\u5E74`
    )
  },
...
const cWeek = solarLunar.nStr1[nWeek]
    if (nWeek == 0) {
      nWeek = 7
    } // 数字表示周几顺应天朝周一开始的惯例
    // 农历年
    ...
    // 天干地支处理
    const sm = m - 1
    const term3 = solarLunar.getTerm(year, 3) // 该农历年立春日期
    let gzY = solarLunar.toGanZhi(year - 4) // 普通按年份计算，下方尚需按立春节气来修正
